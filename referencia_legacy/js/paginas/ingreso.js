class FormularioIngreso {
  ...
  usarPrueba() {
    if (this.correo) {
      this.correo.value = 'comprador@surcos.pa';
    }

    if (this.clave) {
      this.clave.value = 'Surcos123!';
    }
  ...
  actualizarVista() {
    const correo = String(this.correo?.value || 'comprador@surcos.pa').trim();
    const tieneClave = Boolean(String(this.clave?.value || '').trim());

    if (this.vistaCorreo) {
      this.vistaCorreo.textContent = correo || 'comprador@surcos.pa';
    }
